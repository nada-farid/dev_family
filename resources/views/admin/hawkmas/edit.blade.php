...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.hawkma.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.hawkmas.update", [$hawkma->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.hawkma.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $hawkma->title) }}" required>
                @if($errors->has('title'))
                    <div class="invalid-feedback">
                        {{ $errors->first('title') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.hawkma.fields.title_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="file">{{ trans('cruds.hawkma.fields.file') }}</label>
...
<script>
    ...
    init: function () {
@if(isset($hawkma) && $hawkma->file)
      var file = {!! json_encode($hawkma->file) !!}
...
<script>
    ...
    init: function () {
@if(isset($hawkma) && $hawkma->icon)
      var file = {!! json_encode($hawkma->icon) !!}
          this.options.addedfile.call(this, file)
