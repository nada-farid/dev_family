...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.videos.update", [$video->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="image">{{ trans('cruds.video.fields.image') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.video.fields.image_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="url">{{ trans('cruds.video.fields.url') }}</label>
                <input class="form-control {{ $errors->has('url') ? 'is-invalid' : '' }}" type="text" name="url" id="url" value="{{ old('url', $video->url) }}" required>
                @if($errors->has('url'))
                    <div class="invalid-feedback">
                        {{ $errors->first('url') }}
                    </div>
...
@if(isset($video) && $video->image)
      var file = {!! json_encode($video->image) !!}
          this.options.addedfile.call(this, file)
